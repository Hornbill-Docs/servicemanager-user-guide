...
An advanced [Service Manager setting](/servicemanager-config/advanced-tools-and-settings/application-settings) is available to control whether email contacts are set for automatic inclusion by default when they are added as an email contact.

* **app.email.request.autoIncludeEmailContactsInResponses**
...
Each contact in the list displays a toggle that sets if they are automatically included or excluded from a new email.

* **Enabled**. Email contacts showing a blue enabled toggle will be included in the relevant `To`, `Cc`, or `Bcc` field of a new email.
* **Disabled**. Email contacts showing a gray disabled toggle will be excluded from a new email.

![Excluding Contacts](/_books/servicemanager-user-guide/images/request-contact-exclude.png)

Clicking on the toggle will switch  between being included or excluded.  Any changes to these are saved and will still apply when the request is next viewed.
