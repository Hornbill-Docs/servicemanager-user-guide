...
When applying an email to a request, you'll see the new HAi Icon in the attachments header. Simply click the HAi icon on the right-hand side to process the attachments.
...
2. HAi processes the email attachments, descriptions for each are generated and any types marked as not wanted are unchecked, this value is audited for reporting on at a later stage
...
It's possible to enable auto updates for Email Processing so that the email is processed automatically when opening an email to attach to a request
